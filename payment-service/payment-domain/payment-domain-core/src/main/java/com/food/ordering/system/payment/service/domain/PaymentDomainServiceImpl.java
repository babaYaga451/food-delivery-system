...
public class PaymentDomainServiceImpl implements PaymentDomainService{
  ...
  private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories,
      List<String> failureMessages) {
    Money totalCreditAmount = getTotalHistoryAmount(creditHistories, TransactionType.CREDIT);
    Money totalDebitAmount = getTotalHistoryAmount(creditHistories, TransactionType.DEBIT);

    if (totalDebitAmount.isGreaterThan(totalCreditAmount)) {
      log.error("Customer with id: {} doesn't have enough credit as per credit history",
          creditEntry.getCustomerId().getValue());
      failureMessages.add("Customer with id: " + creditEntry.getCustomerId().getValue() +
              " doesn't have enough credit as per credit history");
    }
  ...
  private com.food.ordering.system.domain.valueObject.Money getTotalHistoryAmount(
